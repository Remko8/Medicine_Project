...
namespace Medicine_Project
{
    public partial class AddTemperature : Form
    {
        ...
        {
            InitializeComponent();
            string lastDateS = DateTime.Now.ToString();
            lastDateS = lastDateS.Remove(lastDateS.Count() - 3);

            if (Data.UserTemperatures.Count > 0)
                lastDateS = Data.UserTemperatures[Data.UserTemperatures.Count - 1][2] + " " + Data.UserTemperatures[Data.UserTemperatures.Count - 1][3];
        ...
        {
            TempTXT.Text = TempTXT.Text == "" ? "0" : TempTXT.Text;
