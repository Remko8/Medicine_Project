...
namespace Medicine_Project
{
    public partial class Patient : Form
    {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                if (temperatures.Count >= 9)
                {
                    ...
                    if (ninePlus >= 9)
                    {
                        ...
                        var ntmp = model.NormalizeRow(ftmp);
                        float pred;
                        if (isNN)
                        {
                            pred = model.Predict(ntmp);
                        }
                        else
                        {
                            var predBool = model2.Predict(ntmp);
                            pred = predBool ? 1 : 0;
                        }
        ...
        private void KValComboBox_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            if (KValComboBox != null)
            {
                kNN = KValComboBox.SelectedIndex + 1;
